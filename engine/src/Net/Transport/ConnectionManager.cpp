...
void ConnectionManager::stop() {
	...
	if (tcpListenSocket) tcpListenSocket->close();
...
PeerId ConnectionManager::connect(const Address& address) {
	...
	ByteBuffer buf;
	PacketHeader hdr;
	hdr.packetType = PacketType::ConnectRequest;
	hdr.serialize(buf);
...
void ConnectionManager::disconnect(PeerId peerId) {
	...
	if (peer.tcpSocket) peer.tcpSocket->close();
	peer.state = PeerState::Disconnected;
...
void ConnectionManager::pollTCPAccept() {
	...
	BT_LOG(
		"ConnectionManager: TCP accepted from {} (peerId {})",

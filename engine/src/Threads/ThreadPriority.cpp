...
bool setCurrentThreadPriority(ThreadPriority priority) noexcept {
	...
	switch (priority) {
		case ThreadPriority::Lowest:      qos = QOS_CLASS_BACKGROUND;       break;
		case ThreadPriority::BelowNormal: qos = QOS_CLASS_UTILITY;          break;
		case ThreadPriority::Normal:      qos = QOS_CLASS_DEFAULT;           break;
		case ThreadPriority::AboveNormal: qos = QOS_CLASS_USER_INITIATED;   break;
		case ThreadPriority::Highest:     qos = QOS_CLASS_USER_INTERACTIVE; break;
	}
	...
	int niceValue = 0;
	switch (priority) {
		case ThreadPriority::Lowest:      niceValue =  10; break;
		case ThreadPriority::BelowNormal: niceValue =   5; break;
		case ThreadPriority::Normal:      niceValue =   0; break;
		case ThreadPriority::AboveNormal: niceValue =  -5; break;
		case ThreadPriority::Highest:     niceValue = -10; break;
	}
...
bool setAudioThreadPriority() noexcept {
		...
		BT_DEBUG("Threads: setAudioThreadPriority is a no-op on Windows. Construct an MmcssScope object in your thread.");
	#endif
	return true;
